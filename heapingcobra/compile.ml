...
and compile_prim2 op e1 e2 si env def_env =
  ...
  if op = StructEqual then
    let stack_adjust = Const(if (si) mod 2 = 0 then 8 * (si + 2) else 8 * (si + 3)) in
    (* Set up args according to C's calling convention *)
    e1is @ [IMov(Reg(RDI), Reg(RAX))] @ e2is @ [IMov(Reg(RSI), Reg(RAX));
    (* Call a C function that returns a boolean or throws an error on cycles *)
    ISub(Reg(RSP), stack_adjust); ICall("struct_eq"); IAdd(Reg(RSP), stack_adjust)]
    (*
    If I were to do this in assembly, it would be much more efficient.
    Instead of creating a new data structure to store visited arrays for clean up,
    I could store them directly on the stack.
    Not today, though.
    *)
  else
  ...
  @ e2is @
  [IMov(Reg(RBX), Reg(RAX)); IMov(Reg(RAX), stackloc si)]
  @ match op with

...
let rec tc_e (e : expr) (env : (string * typ) list) (def_env : def_env) (typ_env : typ_env) : typ =
  ...
  | ESet(var, value) -> begin match find env var with
      | Some(typ) when List.mem typ [tc_e value env; TUnknown] -> typ
  ...
  | ESetIndex(arr, i, value) -> 
    ...
    if arr = TArray && i = TNum then value else failwith "Type mismatch"
...
let tc_def def_env def typ_env =
  ...
  let ret_typ = match ret_typ with
  | TName(_) -> convert_alias_typ ret_typ typ_env
  | _ -> ret_typ in

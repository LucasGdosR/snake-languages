...
let rec well_formed_e (e : expr) (env : string list) : string list =
  ...
  (* I decided that one variable declared in a let is considered bound to the following variables in the same let.
     This is different from the specification. Look at copperhead for an alternative implementation. *)
  | ELet(bindings, body) -> 
    let rec check_bindings bs seen_ids expanded_env =
      (match bs with
      | [] -> check_body body expanded_env
      | (id, e)::rest ->
        if List.mem id seen_ids then
          ["Multiple bindings for variable identifier " ^ id]
          @ well_formed_e e expanded_env
          @ check_bindings rest seen_ids expanded_env
        else
          well_formed_e e expanded_env
          @ check_bindings rest (id::seen_ids) (id::expanded_env)
      ) in
    check_bindings bindings [] env
...
and well_formed_def (def : def) =
  let f_name, args, body = match def with
